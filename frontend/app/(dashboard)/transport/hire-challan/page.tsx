...
export default function HireChallansPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const [challansRes, ownersRes, seriesRes, manualRangesRes] = await Promise.all([
        ...
        apiClient<any>("/api/v1/settings/series/manual-ranges/HIRE_CHALLAN").catch(() => null),
      ]);
  ...
  const handleCreate = async (values: Record<string, any>) => {
    ...
    if (isManualSeries && !values.challan_number) {
  ...
  const openCreateDrawer = () => {
    ...
    const initialChallanNo = rangeToUse?.available_options?.[0]?.value || seriesInfo?.next_number_formatted || "";
    if (initialRangeId) {
    ...
    setFormInitialValues({
      ...
      challan_number: initialChallanNo,
    });
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <Form
          ...
          submitLabel={!isManualSeries && seriesInfo && !seriesInfo.configured ? "Series Configuration Required" : "Issue Challan"}
