...
/**
 * All original navigation modules and options as defined across the system
 */
...
export function Sidebar() {
  ...
  useEffect(() => {
    async function loadNavigation() {
      try {
        ...
        // Retain ALL_NAVIGATION_MODULES fallback so all options remain present
      }
    }
    loadNavigation();
  }, [pathname]);
  ...
  return (
    ...
    >
      {/* Brand Header */}
      <div className="flex h-16 items-center justify-between px-4 border-b border-[#E4E7EC]">
        {!isCollapsed && (
          <Link href="/" className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-control bg-[#172033] flex items-center justify-center text-white font-bold tracking-wider shadow-xs relative">
              <span className="text-white text-sm">P</span>
              <span className="w-1.5 h-1.5 rounded-full bg-[#C9A227] absolute top-1 right-1" />
            </div>
            <div>
              <div className="flex items-center gap-1">
                <span className="text-sm font-bold tracking-tight text-[#172033]">
                  Panther<span className="text-[#C9A227]">TMS</span>
                </span>
              </div>
              <span className="block text-[10px] text-[#667085] -mt-0.5 font-medium tracking-tight">
        ...
        {isCollapsed && (
          <div className="w-8 h-8 mx-auto rounded-control bg-[#172033] flex items-center justify-center text-white font-bold tracking-wider relative">
        ...
        <button
          ...
          className="p-1 rounded-control text-[#98A2B3] hover:text-[#172033] hover:bg-[#F2F4F7] transition-colors"
      ...
      {/* Dynamic / Full Navigation Groups */}
          ...
          return (
            <div key={group.id} className="space-y-0.5">
              ...
              <button
                ...
                className={cn(
                  "flex w-full items-center justify-between px-2.5 py-1.5 rounded-control text-xs font-medium transition-colors",
                  hasActiveChild
                    ? "text-[#172033] font-semibold bg-[#F2F4F7]"
                    : "text-[#667085] hover:bg-[#F7F8FA] hover:text-[#172033]"
                )}
                title={isCollapsed ? group.title : undefined}
              >
                <div className="flex items-center gap-2.5">
                  <span className={cn(hasActiveChild ? "text-[#C9A227]" : "text-[#98A2B3]")}>
                    {icon}
                  </span>
                    ...
                    return (
                      <Link
                        ...
                        className={cn(
                          ...
                          isActive
                            ? "bg-[#F2F4F7] text-[#172033] font-semibold border-l-[3px] border-[#C9A227] rounded-l-none shadow-xs"
      ...
      {!isCollapsed && (
        <div className="p-3 border-t border-[#E4E7EC]">
          <div className="rounded-control bg-[#ECFDF3] border border-[#A6F4C5] p-2 flex items-center gap-2">
            <Shield className="w-3.5 h-3.5 text-[#16A34A] shrink-0" />
            <div className="leading-tight">
              <span className="text-[11px] font-semibold text-[#16A34A] block">
                RBAC Permissions Active
              </span>
              <span className="text-[10px] text-[#667085]">
                Dynamic query-driven navigation
              </span>
            </div>
          </div>
        </div>
